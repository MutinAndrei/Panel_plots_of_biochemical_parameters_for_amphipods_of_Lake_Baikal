...
Lactate <- read.xlsx("Table S3_Raw data.xlsx", sheet = 12)
...
#make data part is factors (LDH)
Lactate$Temperature <- as.factor(Lactate$Temperature)
Lactate$Species <- as.factor(Lactate$Species)
Lactate$Parameter <- as.factor(Lactate$Parameter)
Lactate$Depth <- as.factor(Lactate$Depth)
...
#split table at Species factor (Lactate)
FlavusLactate <- Lactate[Lactate$Species == "O. flavus", ]
...
#plot for Lactate O. flavus
FlavusLactate[,3] <- factor(FlavusLactate[,3], levels=unique(FlavusLactate[,3]))
FlavusLactatecoluor <- c("darkorange", "darkorange", "darkorange", "gray40", "darkorange", "darkorange",
                         "darkorange", "darkorange", "darkorange", "darkorange", "darkorange", "darkorange",
                         "darkorange")
ggplot(FlavusLactate, 
       aes(x = Temperature, y = Value, ymin = 0)) +
  geom_boxplot(outlier.alpha = 0, alpha = 0.5, fill = FlavusLactatecoluor, width = 0.6) +  
  geom_point(size = 2, alpha = 1, aes(group = Temperature, col = Depth, shape = Depth), 
             position = position_jitterdodge(jitter.width = 0.5)) +
  labs(fill = "Температура",
       col = "Depth, m:",
  ...
  theme(plot.title = element_text(hjust = 0.5)) +
  geom_vline(xintercept=c(3.5,4.5), linetype = "dashed", linewidth = 0.6) +
  annotate("segment", x = 5, xend = 7, y = 1.5070, yend = 1.5070, colour = "firebrick2", size=1.2, arrow=arrow(length = unit(0.2, "cm"))) +
  annotate("segment", x = 3, xend = 1, y = 1.5070, yend = 1.5070, colour = "dodgerblue2", size=1.2, arrow=arrow(length = unit(0.2, "cm"))) +
  annotate("text", x = c(2), y = c(1.825), label = "Temperature", cex=4.5, col = "black") +
  annotate("text", x = c(2), y = c(1.640), label = "decrease", cex=4.5, col = "black") +
  annotate("text", x = c(6), y = c(1.825), label = "Temperature", cex=4.5, col = "black") +
  annotate("text", x = c(6), y = c(1.640), label = "increase", cex=4.5, col = "black") +
  annotate("text", x = c(4), y = c(1.550), label = "Control", cex=3, col = "black") + 
  theme(text=element_text(color="black"),axis.text=element_text(color="black")) + 
  theme(text=element_text(color="black"),axis.text=element_text(color="black")) + 
  annotate("text", x = c(11), y = c(.75), label = substitute(paste(bold("*"))), cex=10, col = "black") + 
  annotate("text", x = c(12), y = c(1.25), label = substitute(paste(bold("*"))), cex=10, col = "black") + 
  annotate("text", x = c(13), y = c(1.25), label = substitute(paste(bold("*"))), cex=10, col = "black") +
  theme(text=element_text(size=18, family="Arial")) +
  theme(legend.position = c(0.75, 1.0515),
        legend.direction = "horizontal") +
  theme(plot.title = element_text(hjust = 0.01)) + theme(legend.background=element_rect(fill = alpha("white", 0))) + 
  theme(
    axis.line = element_line(linewidth = 0.4, color="black")) +
  ylim(0,2)
#save plot Lactate
ggsave("2024-10-10_lactate flavus.png", width = 20, height = 11, units = "cm", dpi = 600)
